...
# Should be treated as a Singleton to exploit cache and reduce API calls
class RbAPI: 
    ...
    MODEL_APPLIANCES = 're_features_v3'
    ...
    def get_features(self, image_url: str = DEMO_IMAGE_URL):
        ...
        return self._cached_requests[image_url][RBAI_MODEL_FEATURES]


    def get_appliances(self, image_url: str = DEMO_IMAGE_URL):
        self._cache(image_url)
        return self._cached_requests[image_url][RBAI_MODEL_APPLIANCES]


    def get_room_type(self, image_url: str = DEMO_IMAGE_URL):
        self._cache(image_url)
        return self._cached_requests[image_url][RBAI_MODEL_ROOM_TYPE]


    def get_condition(self, image_url: str = DEMO_IMAGE_URL):
        self._cache(image_url)
        return self._cached_requests[image_url][RBAI_MODEL_CONDITION]
    ...
    def _api_call(self, image_url=RBAI_DEMO_URL, model_ids=ALL_MODELS):
        params = {
            'image_url': image_url,
            'model_id': ",".join(model_ids),
            'client_key': RBAI_CLIENT_KEY
        }

        response = requests.get(RBAI_BASE_URL, params = params)
